...
def forward(args):
    ...
    for i_subject in range(15):
        ...
        for i_slice in range(num_slices):
            ...
            plt.subplot(1, 4, 4)
            plt.title("diff")
            plt.imshow(np.abs(recon - target))
            plt.show()
            print(np.mean(recon * mask ), np.mean(target * mask))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-lp', '--leaderboard_data_path', type=Path, default='./leaderboard_data/')
    """
