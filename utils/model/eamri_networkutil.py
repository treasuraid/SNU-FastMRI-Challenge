    ...
    def __init__(self, in_feat=2, mid_feat=32, WH=0, M=2, G=8, r=16, stride=1, L=16):
        ...
        for i in range(M):  # for each branch
            self.split_convs.append(
                nn.Sequential(
                    nn.Conv2d(mid_feat, mid_feat, kernel_size=3, stride=stride, padding=1 + i, dilation=1 + i,
                              bias=False),  # use 3*3 kernel
    ...
    def __init__(self,
        ...
        for i in range(0, self.denselayer):  # 0,1
            if (useOri):
                ...
                # tempLayer = new_denseBlockLayer(indim=inChannel + growthRate * i, middim=inChannel,
                #                                 outdim=inChannel + growthRate * i)
                raise NotImplementedError("new_denseBlockLayer is not implemented yet.") # TODO
...
class DAM(nn.Module):
    ...
    def __init__(self, inChannel=2,
        ...
        if (transition > 0):
            ...
            self.outConv = convLayer(int((fNum + growthRate * (layer - 2)) * transition), self.outChannel,
                                     activ=activation)
...
class denseBlock(nn.Module):
    def __init__(self, inChannel=64, outChannel=64, kernelSize=3, growthRate=16, layer=4, inceptionLayer=False,
        ...
        for i in range(0, layer):
            tempLayer = denseBlockLayer(inChannel + growthRate * i, growthRate, kernelSize, inceptionLayer, dilate,
                                        activ)
            dilate = dilate * dilateMulti
            templayerList.append(tempLayer)
        self.layerList = nn.ModuleList(templayerList)
        self.outputLayer = denseBlockLayer(inChannel + growthRate * layer, outChannel, kernelSize, inceptionLayer, 1,
                                           activ)
...
class denseBlockLayer_origin(nn.Module):
    ...
    def __init__(self, inChannel=64, outChannel=64, kernelSize=3, bottleneckChannel=64, dilateScale=1, activ='ReLU'):
        ...
        if (activ == 'LeakyReLU'):
            ...
            self.relu2 = nn.ReLU()
        self.conv2 = nn.Conv2d(bottleneckChannel, outChannel, kernelSize, padding=dilateScale,
                               dilation=dilateScale)  # #*3 conv
...
class denseBlock_origin(nn.Module):
    def __init__(self, inChannel=64, kernelSize=3, growthRate=16, layer=4, bottleneckMulti=4, dilationLayer=False,
                 activ='ReLU'):
        ...
        templayerList = []
        for i in range(0, layer):
            tempLayer = denseBlockLayer_origin(inChannel + growthRate * i, growthRate, kernelSize,
                                               bottleneckMulti * growthRate, dilate, activ)
            dilate = dilate * dilateMulti
            templayerList.append(tempLayer)
        self.layerList = nn.ModuleList(templayerList)
